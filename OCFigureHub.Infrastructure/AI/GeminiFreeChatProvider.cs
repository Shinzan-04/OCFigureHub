...
public class GeminiFreeChatProvider : BaseGeminiChatProvider
{
...
7. Tuyệt đối không hỏi hoặc lưu trữ thông tin nhạy cảm của người dùng.";
    ...
    public override string Name => "GeminiFree";

    public GeminiFreeChatProvider(
        HttpClient httpClient,
        ILogger<GeminiFreeChatProvider> logger,
        string apiKey)
        : base(httpClient, logger, apiKey, "gemini-flash-lite-latest")
    {
    }
    ...
    protected override List<object> BuildContents(AiChatProviderRequest request)
    {
        ...
        // 1. Context (Products) - provided as a separate system-like message if exists
