...
export function AdminAnalytics() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [dashData, catDist, actData, topProds] = await Promise.all([
          analyticsApi.dashboardStats(period),
          analyticsApi.categoryDistribution(period).catch(() => []),
          analyticsApi.activity(period).catch(() => []),
          analyticsApi.topProducts(period, 8).catch(() => [])
        ]);
        setStats(dashData);
  ...
  // Top products by name is now handled by TopProducts API
  ...
  return (
    <div className="space-y-5 pb-20 md:pb-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Products', value: stats.totalProducts.toLocaleString(), change: '', color: '#8B5CF6', icon: Eye },
          { label: 'Downloads', value: stats.totalDownloads.toLocaleString(), change: '', color: '#10B981', icon: Download },
          { label: 'Users', value: stats.totalUsers.toLocaleString(), change: '', color: '#06B6D4', icon: Globe },
      ...
      {/* Views + Downloads Activity */}
      <div className="rounded-xl p-5" style={{ background: '#111111', border: '1px solid #262626' }}>
        <h3 style={{ color: '#fff', fontSize: 14, fontWeight: 600, marginBottom: 16 }}>Views & Downloads (This {period.charAt(0).toUpperCase() + period.slice(1)})</h3>
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={activityData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="views" name="Views" stroke="#8B5CF6" strokeWidth={2} dot={false} />
            <Line type="monotone" dataKey="downloads" name="Downloads" stroke="#10B981" strokeWidth={2} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl p-5" style={{ background: '#111111', border: '1px solid #262626' }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={trafficSources} margin={{ top: 0, right: 20, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1F1F1F" />
              <XAxis dataKey="name" tick={{ fill: '#ccc', fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: '#666', fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="value" name="Products" fill="#8B5CF6" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="rounded-xl" style={{ background: '#111111', border: '1px solid #262626' }}>
        <div className="px-5 py-4" style={{ borderBottom: '1px solid #262626' }}>
          <h3 style={{ color: '#fff', fontSize: 14, fontWeight: 600 }}>Products (Top Downloads)</h3>
        </div>
